...
- What is the mean departure delay time, among available values? 12.63907
...
- Which carriers are at the top of both `mean` and `median` departure delay? wn,f9
- Which carriers differ in the two lists? ua,vx
- What do negative values of `dep_delay` mean? left early
...
test_iqr <- quantile(v_test_iqr,0.75) - quantile(v_test_iqr,0.25)
```
...
- Which carrier has the biggest difference between its `sd` and `IQR`? What might this mean? HA outliers
...
```{r ex-cor-line}
slope <- 0.02 # Play with this value; observe the correlation
df_line <-
