...
```{r q6-task}
 df_q6a <- flights %>% filter(dest %in% c("JFK","LGA","EWR")) 
df_q6a 
 df_q6b <- flights %>% filter(origin %in% c("JFK","LGA","EWR"))
 df_q6b
```
...
```{r q7-task}
 df_q7 <- flights %>% 
  arrange(desc(air_time)) %>% select(air_time, dest,everything())
df_q7
```
